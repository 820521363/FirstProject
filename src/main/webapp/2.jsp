...
$(function() { 
 ...
 out.print("<script>console.log('"+location_id+"');</script>");
...
if ((submit!=null) && (ChkStrEq(submit,"Print Tag"))&&!ChkStrEq(DocType, "OWP")) {
		...
		if (ChkStrEq(yearText,"null")){
		  ...
		  else if(monthText.length()==1&&ChkStrEq(monthText,"0"))
		  {
			  monthText="00";
		  }
		...
		{
			dayText="00";
		}else if(dayText.length()==1&&ChkStrEq(dayText,"0"))
		  {
			dayText="00";
		  }
		...
		if ((JsonData==null) || (ChkStrEq(JsonData,""))) {	
	     ...
	     JsonData="{\"funcname\":\"" + submit + "\",\"allinputpara\":\"{\\\"dummy\\\" :\\\"||" +TagType+ "||"+DocType+ "||" +realDocRef + "||" + yearText + "||" + monthText + "||" + dayText + "||" + terminal_id +  "||" + user_id+"||"+smartics2+"||"+location_id+"\\\"}\"}";
			}	
	...
	//	out.println ("<script>alert('here');</script>");
		...
		{
			tagno=alloutpara.replaceFirst("TQWSCANDOCPRINT\\|\\|"+ terminal_id.toString() + "\\|\\|"+request.getParameter("trn")+"\\|\\|","");
			rediract="2-0.jsp";
		}
...
// group appointment vb -> Json Result : {"alloutpara":"MSG=TQWSCANDOCRESULT||terminal id||TRN||no. of record||doc1 type||doc1 num||DoB(yyyy-mm-dd)||doc1 is printable ("0" - printable, "1" - fail due to inccorect time slot, "2" - fail to print due to already register)||doc2 type||doc2 num||doc2 is printable||...||time slot","funcname":"PrintTag","return_code":"0"}
		...
		{
			duplicate=true;
			//alloutpara = alloutpara.replaceFirst("TQWDUPLICATEREC\\|\\|"+ terminal_id.toString() + "\\|\\|","");
		...
		//group ticket
			...
			alloutpara = alloutpara.replaceFirst("TQWSCANDOCRESULT\\|\\|" + terminal_id.toString() + "\\|\\|","");
		...
		if(duplicate)//XXXX
		{
			...
			out.println("<input type='hidden' name='TagType'  value='" + TagType + "'>");
 			out.println("<input type='hidden' name='TerminalID'  value='" + terminal_id + "'>");
			out.println("<input type='hidden' name='monthText'  value='" + monthText + "'>");
			out.println("<input type='hidden' name='UserID'  value='" + user_id + "'>");
	...
	//
		if (monthText==null || ChkStrEq(monthText,"")) monthText = "00";
		else if(monthText.length()==1&&ChkStrEq(monthText,"0"))
		  {
			monthText="00";
		  }
		if (dayText==null || ChkStrEq(dayText,"")) dayText = "00";	
		else if(dayText.length()==1&&ChkStrEq(dayText,"0"))
		  {
			dayText="00";
		  }	
								...
								<table width="100%" border="0" cellspacing="0" cellpadding="0">
                                  <tr>
                                    ...
                                    <td><font size="5"><b>
                                     ...
                                     <% 
                                    String owpShow="";
                                    if(location_id.contains("RKO"))
                                    {
                                 owpShow="<td><font size='5'><b><input type='radio' name='DocType' id='DocType' class='DocType'    value='OWP' style = 'width: 25px; height: 25px;' " ;
                               if (DocType!=null && ChkStrEq(DocType,"OWP")) 
                               {
                                     owpShow=owpShow+  "checked"+" onClick='EnableTextbox(1)'> OWP </b></font></td>";
                                    }
                               else
                               {
                            	   owpShow=owpShow+" onClick='EnableTextbox(1)'> OWP </b></font></td>";
                               }
                                      out.print(owpShow);
                               }
                                      %>
