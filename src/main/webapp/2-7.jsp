...
out.print("<script>console.log('"+alertSideString+"');</script>");
	...
	if ((DocRefSubmit!=null) && (ChkStrEq(DocRefSubmit,"OK"))) {
		...
		if ((yearText=="") || (monthText=="") || (dayText=="")) {
			...
			if(monthText.length()<2)monthText="0"+monthText;
	...
	if (((TagNoSubmit!=null) && (ChkStrEq(TagNoSubmit,"OK")))||((DocRefSubmit!=null) && (ChkStrEq(DocRefSubmit,"OK")))){
		try
		{
				...
				try
				{
					...
					try
					{
						...
						if ((TagNoSubmit!=null) && (ChkStrEq(TagNoSubmit,"OK"))){
					     ...
					     JsonData="{\"funcname\":\"" + TagNoSubmit + "\",\"allinputpara\":\"{\\\"dummy\\\" :\\\"||" +TagNo+"||" +date+"||" +location_id+"\\\"}\"}";
					     ...
					     if(ChkStrEq(DocType, "1")||ChkStrEq(DocType, "4"))
					     {
							...
							sql="call sp_SearchTagNo('', '" + DocType + "', '" + DocRef + "', '" + DoB +"', '" +location_id+ "')"; 		
						     JsonData="{\"funcname\":\"" + DocRefSubmit + "\",\"allinputpara\":\"{\\\"dummy\\\" :\\\"||" +DocType+"||" +DocRef+"||"+DoB+ "||"+location_id+"||"+terminal_id+"||"+user_id+"\\\"}\"}";
								//las1.saveRequestOrReponseParamsLog(JsonData, loginData);
						...
						while(rs.next())
						{
						   ...
						   //Result_tag_type=rs.getString("tag_type");
							...
							if (Result_tag_status==null) {Result_tag_status="";}
						}		
				    	 String Result="{\"alloutpara\":\"MSG=SEARCHTAGRESULT||"+Result_tag_id+"||"+apm_date+"||"+Result_tag_status+"||"+arn+"||"+est_waiting_time+"||"+tag_issue_time+"||"+deactivate_reason+"\",\"funcname\":\"SearchTag\",\"return_code\":\"0\"}";
				 	//	las1.saveRequestOrReponseParamsLog(Result, loginData);
						...
						if(ChkStrEq(location_id, "")||(location_id==null))
						{
						...
						out.println("<input type='hidden' name='Result_tag_id' id='Result_tag_id' value='" + Result_tag_id + "'>");
				//		out.println("<input type='hidden' name='Result_tag_type' id='Result_tag_type' value='" + Result_tag_type + "'>");
						out.println("<input type='hidden' name='Result_tag_status' id='Result_tag_status' value='" + Result_tag_status + "'>");
						out.println("<input type='hidden' name='location_id' id='location_id' value='" + location_id + "'>");
						out.println("<input type='hidden' name='terminal_id' id='terminal_id' value='" + terminal_id + "'>");
						out.println("<input type='hidden' name='apm_date' id='apm_date' value='" + apm_date + "'>");
						out.println("<input type='hidden' name='smartics2' id='smartics2' value='" + smartics2 + "'>");
						out.println("<input type='hidden' name='user_id' id='user_id' value='" + user_id + "'>");
						out.println("<input type='hidden' name='arn' id='arn' value='" + arn + "'>");
						out.println("<input type='hidden' name='est_waiting_time' id='est_waiting_time' value='" + est_waiting_time + "'>");
						 
						out.println("<input type='hidden' name='tag_issue_time' id='tag_issue_time' value='" + tag_issue_time + "'>");
						out.println("<input type='hidden' name='deactivate_reason' id='deactivate_reason' value='" + deactivate_reason + "'>");
						out.println("</form>");
	...
	if (TagNo=="" || TagNo==null || ChkStrEq(TagNo,"NULL")) TagNo="";
	if (DocType=="" || DocType==null || ChkStrEq(DocType,"NULL")) DocType="";
...
<table width="100%" border="0" cellspacing="5" cellpadding="5">
      ...
      <table width="100%" border="0">
        <tr> 
          <td><b><font size="3">Login &gt;&gt; Operation Selection &gt;&gt; TQW 
            - SMARTICS-2 &gt;&gt; Tag Status Search</font></b></td>
          <td width="100" bgcolor="#FFFF99"> 
            <div align="center"><b>TQW020</b></div>
          </td>
        </tr>
      </table>
    </td>
  ...
  <tr> 
    <td colspan="6"><b><font size="6"><br>
		...
		<form name="input" id="input" action="2-7.jsp" method="post"  onsubmit="return TagNoValidation();">
        <table width="880" border="0" cellpadding="5" cellspacing="3" align="center" bordercolor="#FFFFCC">
          <tr> 
            <td colspan="2" bgcolor="#FFFFCC"><font size="5"><b>Record Search 
              by Tag Number</b></font></td>
          </tr>
          <tr> 
            <td><font size="5"><b>Input Tag No.: </b></font><font size="5"><input type="text" id="TagNo" name="TagNo"   style="font-size: 15pt"   autofocus maxlength="6" size="16" value="<%=TagNo%>"></font></td>
            <td> 
              <div align="right"><font size="5">
				<input type="hidden" name="searchkey" id="searchkey" value="tagno">
                <input type="submit" name="TagNoSubmit"  id="TagNoSubmit" style="height:30px; width:70px; font-size: 12pt" value="OK">
                <input type="reset" name="reset" value="Clear" style="height:30px; width:70px; font-size: 12pt" >
                </font></div>
            </td>
          </tr>
        </table>
	  ...
	  <form name="input2" id="input2" action="2-7.jsp" method="post" onsubmit="return FormValidation();">
	  <table width="880" border="0" cellspacing="3" cellpadding="5" align="center">
        <tr bgcolor="#FFFFCC"> 
          <td height="12" colspan="3"><font size="5"> <b>Record Search by Document 
            Number</b></font></td>
        </tr>
        <tr> 
          <td height="12" colspan="3"><font size="5"><b> 
            <input type="radio" class="DocType" name="DocType" id="DocType" maxlength="9" value="1" style = "width: 25px; height: 25px;" <%if (DocType!=null && (ChkStrEq(DocType,"HKID")||ChkStrEq(DocType,""))) {out.println("checked");}%> onClick="EnableTextbox(0)">
            HKID 
            <input type="radio" class="DocType"  name="DocType" id="DocType" value="2" style = "width: 25px; height: 25px;" <%if (DocType!=null && ChkStrEq(DocType,"TravelDoc")) {out.println("checked");}%> onClick="EnableTextbox(1)">
            Travel Document 
            <input type="radio" class="DocType"  name="DocType" id="DocType" value="4" style = "width: 25px; height: 25px;" <%if (DocType!=null && ChkStrEq(DocType,"BirthCert")) {out.println("checked");}%> onClick="EnableTextbox(0)">
            Birth Certificate 
            <input type="radio" class="DocType"   name="DocType" id="DocType" value="3" style = "width: 25px; height: 25px;" <%if (DocType!=null && ChkStrEq(DocType,"ARN")) {out.println("checked");}%> onClick="EnableTextbox(0)">
            ARN
            <%
                  //
                  String owpShow="";
                  ...
                  {
               owpShow="<input type='radio'  class='DocType'    name='DocType'   id='DocType'  value='6' style = 'width: 25px; height: 25px;' " ;
             if (DocType!=null && ChkStrEq(DocType,"6")) 
             {
                   owpShow=owpShow+" checked  onClick='EnableTextbox(1)'> OWP";
               }
             else
             {
          	   owpShow=owpShow+" onClick='EnableTextbox(1)'> OWP ";
             }
                    out.print(owpShow);
                }
                  //
        ...
        <tr> 
          ...
          <td><div><font size="5"><b> 
             <input type='text' class='arn' id='arn1' name='arn1' onKeyup="autotab(this, document.input2.arn2)" style="font-size: 15pt"  size=4 maxlength=4 /><span class="arn">-</span><input type='text' id='arn2'  name='arn2'  style="font-size: 15pt"  class='arn' size=7  onKeyup="autotab(this, document.input2.arn3)" maxlength=7 /><span class="arn">-</span><input type='text' id='arn3' name="arn3" size='2'  style="font-size: 15pt" onKeyup="autotab(this, document.input2.arncheck)"  class='arn' maxlength=2/><span class="arn">-</span><input type='text' id='arncheck' size='1'  style="font-size: 15pt"   name="arncheck" class='arn' maxlength=1/>
            <input type="text" id="DocRef" name="DocRef"   style="font-size: 15pt"   autofocus maxlength="9" size="18" value="<%=DocRef%>">
              </b></font></div>
             </td>
          <td rowspan="2" valign="bottom"> 
            <div align="right"><font size="5"> 
              <input type="submit" name="DocRefSubmit"  id="DocRefSubmit" style="height:30px; width:70px; font-size: 12pt" value="OK">
              <input type="button" name="clear"  id="clear" value="Clear" style="height:30px; width:70px; font-size: 12pt" >
              </font></div>
          </td>
        </tr>
        <tr> 
          ...
          <td><font size="5"> 
			<input type="text"  id="dayText" name="dayText"  onKeyup="autotab(this, document.input2.monthText)" size="2" maxlength="2" disabled style="font-size: 15pt" value="<%=dayText%>">
			/
            <input type="text"  id="monthText" name="monthText" size="2" onKeyup="autotab(this, document.input2.yearText)" maxlength="2" disabled style="font-size: 15pt" value="<%=monthText%>">
            / 
